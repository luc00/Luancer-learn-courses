...
import {
  ...
  Check, X, ArrowLeft, Volume2, Lock, Upload, RefreshCw, Link2, Apple, Mail, LogOut, Eye, EyeOff,
  Menu, Code, Smartphone, Cpu, Shield, Cloud, Megaphone, Briefcase, LineChart, PenTool, Layout,
  Camera, Clapperboard, Music, PenLine, Brain, HeartPulse, ChefHat, Cog, Building2, Scale,
} from "lucide-react";
...
/* ------------------------------------------------------------------ */
/* ACCOUNT — talks to the same backend for signup / login / password   */
/* reset. Real accounts, real password hashing — Google/Apple need     */
/* your own OAuth app credentials (see server/README.md).              */
/* ------------------------------------------------------------------ */
...
const apiSignup = (name, email, password) => apiCall("/auth/signup", { name, email, password });
const apiLogin = (email, password) => apiCall("/auth/login", { email, password });
const apiForgotPassword = (email) => apiCall("/auth/forgot-password", { email });
const apiResetPassword = (token, password) => apiCall("/auth/reset-password", { token, password });
...
// icona rappresentativa per ciascuna disciplina — usata come "copertina" del corso
const CATEGORY_ICON = {
  "Sviluppo Web": Code,
  "Sviluppo Mobile": Smartphone,
  "Data Science e AI": Cpu,
  "Cybersecurity": Shield,
  "Cloud Computing": Cloud,
  "Marketing Digitale": Megaphone,
  "Business e Management": Briefcase,
  "Finanza e Investimenti": LineChart,
  "Design Grafico": PenTool,
  "UX/UI Design": Layout,
  "Fotografia": Camera,
  "Video Editing e Produzione": Clapperboard,
  "Musica e Produzione Audio": Music,
  "Scrittura e Content Creation": PenLine,
  "Psicologia e Crescita Personale": Brain,
  "Salute e Fitness": HeartPulse,
  "Cucina e Enogastronomia": ChefHat,
  "Ingegneria": Cog,
  "Architettura e Interior Design": Building2,
  "Diritto e Consulenza Legale": Scale,
};

// due toni chiari alternati per categoria, entrambi dentro la palette del brand
const COVER_TINTS = ["#EAF1EB", "#DCE9DE"];
...
function PageNav({ goBack, setView }) {
  return (
    <div className="flex items-center gap-3 mb-5 text-sm" style={{ color: C.muted }}>
      <button onClick={goBack} className="inline-flex items-center gap-1 hover:opacity-70">
        <ArrowLeft size={15} /> Indietro
      </button>
      <span style={{ width: 1, height: 14, background: C.border }} />
      <button onClick={() => setView({ name: "home" })} className="inline-flex items-center gap-1 hover:opacity-70">
        <Menu size={15} /> Menu
      </button>
    </div>
  );
...
function CourseCover({ category, premium, className = "", iconSize = 34 }) {
  const Icon = CATEGORY_ICON[category] || Code;
  const idx = CATEGORIES.indexOf(category);
  const tint = premium ? C.sage900 : COVER_TINTS[idx % COVER_TINTS.length];
  const iconColor = premium ? C.sage200 : C.sage700;
  return (
    <div
      className={`relative flex items-center justify-center overflow-hidden ${className}`}
      style={{
        background: tint,
        backgroundImage: `repeating-linear-gradient(135deg, ${premium ? "#ffffff10" : "#ffffff55"} 0px, ${premium ? "#ffffff10" : "#ffffff55"} 1px, transparent 1px, transparent 14px)`,
      }}
    >
      <div
        className="rounded-full flex items-center justify-center"
        style={{ width: iconSize * 2, height: iconSize * 2, background: premium ? "#ffffff14" : "#ffffff70" }}
      >
        <Icon size={iconSize} style={{ color: iconColor }} strokeWidth={1.6} />
      </div>
    </div>
  );
}
...
function CourseCard({ course, onOpen }) {
  return (
    <button
      ...
      className="text-left rounded-lg overflow-hidden flex flex-col transition-colors"
      style={{ background: C.card, border: `1px solid ${C.border}` }}
    >
      <CourseCover category={course.category} premium={course.premium} className="aspect-[16/9]" />
...
function Header({ view, setView, session, onLogout }) {
  const nav = [
    { key: "home", label: "Home" },
  ...
  return (
    <header className="sticky top-0 z-20 backdrop-blur" style={{ background: `${C.bg}F2`, borderBottom: `1px solid ${C.border}` }}>
      <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        {session ? (
          <div className="flex items-center gap-3">
            <span className="hidden sm:inline text-sm" style={{ color: C.ink }}>Ciao, {session.name.split(" ")[0]}</span>
            <button onClick={onLogout} className="inline-flex items-center gap-1.5 text-sm" style={{ color: C.muted }}>
              <LogOut size={14} /> Esci
            </button>
          </div>
        ) : (
          <Button variant="primary" className="!px-4 !py-2" onClick={() => setView({ name: "login" })}>Accedi</Button>
        )}
      </div>
    </header>
  );
...
function CatalogPage({ courses, setView, goBack, initialCategory }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-10">
      <PageNav goBack={goBack} setView={setView} />
...
function CourseDetailPage({ course, setView, goBack, initialEnrolledLang, paymentCanceled }) {
    ...
    <div>
      <div style={{ background: C.bgAlt, borderBottom: `1px solid ${C.border}` }}>
        <div className="max-w-5xl mx-auto px-6 pt-5 pb-12">
          <PageNav goBack={goBack} setView={setView} />
          <CourseCover category={course.category} premium={course.premium} className="w-full h-44 md:h-56 rounded-lg mb-6" iconSize={44} />
...
function LanguagesPage({ setView, goBack }) {
  const [search, setSearch] = useState("");
...
function LessonPage({ lang, setView, goBack }) {
...
/* ------------------------------------------------------------------ */
/* AUTH — login, crea account, password dimenticata, reimposta         */
/* ------------------------------------------------------------------ */

function TextField({ label, error, ...props }) {
  return (
    <div>
      <label className="text-xs mb-1.5 block" style={{ color: C.muted }}>{label}</label>
      <input
        {...props}
        style={{ border: `1px solid ${error ? "#C24444" : C.border}`, background: C.card }}
        className="w-full px-3 py-2.5 rounded text-sm focus:outline-none"
      />
      {error && <p className="text-xs mt-1" style={{ color: "#C24444" }}>{error}</p>}
    </div>
  );
}
    ...
    <div>
      ...
      <div className="relative">
        <input
          ...
          onChange={onChange}
          style={{ border: `1px solid ${error ? "#C24444" : C.border}`, background: C.card }}
          className="w-full pl-3 pr-10 py-2.5 rounded text-sm focus:outline-none"
        />
        <button type="button" onClick={() => setShow((s) => !s)} className="absolute right-3 top-2.5" style={{ color: C.muted }}>
...
function OAuthButtons({ onUnavailable }) {
  return (
    <div className="space-y-2">
      <button
        ...
        className="w-full py-2.5 rounded-md text-sm font-medium flex items-center justify-center gap-2 hover:opacity-85"
      >
        <span className="w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-bold" style={{ background: C.ink, color: "#fff" }}>G</span>
        Continua con Google
      </button>
      <button
        type="button"
...
function AuthShell({ title, subtitle, children, setView, goBack }) {
  return (
    <div className="max-w-sm mx-auto px-6 py-14">
      <PageNav goBack={goBack} setView={setView} />
      <h1 className="font-serif text-2xl mb-1" style={{ color: C.ink }}>{title}</h1>
      {subtitle && <p className="text-sm mb-7" style={{ color: C.muted }}>{subtitle}</p>}
      {children}
    </div>
  );
}

function LoginPage({ setView, goBack, onLoggedIn }) {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    const errs = {};
    if (!isValidEmail(email)) errs.email = "Inserisci un'email valida.";
  ...
  return (
    <AuthShell title="Accedi" subtitle="Bentornato su Ordinem" setView={setView} goBack={goBack}>
      <OAuthButtons onUnavailable={(p) => setOauthNote(`Il login con ${p} richiede di collegare le tue credenziali OAuth — vedi server/README.md.`)} />
      {oauthNote && <p className="text-xs mt-2" style={{ color: C.muted }}>{oauthNote}</p>}
      <div className="flex items-center gap-3 my-5">
        <div className="flex-1 h-px" style={{ background: C.border }} />
        <span className="text-xs" style={{ color: C.muted }}>oppure</span>
        <div className="flex-1 h-px" style={{ background: C.border }} />
      </div>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {formError && <p className="text-xs" style={{ color: "#C24444" }}>{formError}</p>}
        <Button variant="primary" className="w-full" type="submit" disabled={loading}>
          {loading ? "Accesso in corso…" : "Accedi"}
        </Button>
      </form>
      <div className="flex items-center justify-between mt-4 text-sm">
        <button onClick={() => setView({ name: "forgot" })} style={{ color: C.sage600 }} className="hover:underline">Password dimenticata?</button>
        <button onClick={() => setView({ name: "signup" })} style={{ color: C.sage600 }} className="hover:underline">Crea account</button>
      </div>
    </AuthShell>
  );
}

function SignupPage({ setView, goBack, onLoggedIn }) {
  ...
  async function handleSubmit(e) {
    ...
    if (!isValidEmail(email)) errs.email = "Inserisci un'email valida.";
    if (password.length < 8) errs.password = "Almeno 8 caratteri.";
  ...
  return (
    <AuthShell title="Crea account" subtitle="Accedi a tutti i corsi e alle lingue di Ordinem" setView={setView} goBack={goBack}>
      <OAuthButtons onUnavailable={(p) => setOauthNote(`La registrazione con ${p} richiede di collegare le tue credenziali OAuth — vedi server/README.md.`)} />
      {oauthNote && <p className="text-xs mt-2" style={{ color: C.muted }}>{oauthNote}</p>}
      <div className="flex items-center gap-3 my-5">
        <div className="flex-1 h-px" style={{ background: C.border }} />
        <span className="text-xs" style={{ color: C.muted }}>oppure</span>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <TextField label="Email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} error={errors.email} />
        <PasswordField label="Password" value={password} onChange={(e) => setPassword(e.target.value)} error={errors.password} />
        <PasswordField label="Conferma password" value={confirm} onChange={(e) => setConfirm(e.target.value)} error={errors.confirm} />
        {formError && <p className="text-xs" style={{ color: "#C24444" }}>{formError}</p>}
        <Button variant="primary" className="w-full" type="submit" disabled={loading}>
      ...
      <p className="text-sm mt-4" style={{ color: C.muted }}>
        Hai già un account?{" "}
        <button onClick={() => setView({ name: "login" })} style={{ color: C.sage600 }} className="hover:underline">Accedi</button>
      </p>
    </AuthShell>
  );
}

function ForgotPasswordPage({ setView, goBack }) {
  const [email, setEmail] = useState("");
  ...
  if (sent) {
    return (
      <AuthShell title="Controlla la tua email" setView={setView} goBack={goBack}>
        <div className="p-4 rounded-lg flex gap-3" style={{ background: C.bgAlt }}>
          <Mail size={18} style={{ color: C.sage600 }} className="shrink-0 mt-0.5" />
          <p className="text-sm" style={{ color: C.ink }}>
            Se <strong>{email}</strong> è collegata a un account Ordinem, riceverai a breve un'email con le istruzioni per reimpostare la password. Controlla anche lo spam.
          </p>
        </div>
        <button onClick={() => setView({ name: "login" })} className="text-sm mt-5 hover:underline" style={{ color: C.sage600 }}>Torna al login</button>
      </AuthShell>
    );
  }

  return (
    <AuthShell title="Recupera la password" subtitle="Ti inviamo un link per reimpostarla via email" setView={setView} goBack={goBack}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <TextField label="Email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} error={error} />
        <Button variant="primary" className="w-full" type="submit" disabled={loading}>
          {loading ? "Invio in corso…" : "Invia link di recupero"}
        </Button>
      </form>
      <button onClick={() => setView({ name: "login" })} className="text-sm mt-4 hover:underline" style={{ color: C.sage600 }}>Torna al login</button>
    </AuthShell>
  );
}

function ResetPasswordPage({ token, setView, goBack }) {
  const [password, setPassword] = useState("");
  const [confirm, setConfirm] = useState("");
  const [errors, setErrors] = useState({});
  const [loading, setLoading] = useState(false);
  ...
  if (done) {
    return (
      <AuthShell title="Password aggiornata" setView={setView} goBack={goBack}>
        <p className="text-sm mb-5" style={{ color: C.muted }}>Ora puoi accedere con la tua nuova password.</p>
        <Button variant="primary" onClick={() => setView({ name: "login" })}>Vai al login</Button>
      </AuthShell>
    );
  }

  return (
    <AuthShell title="Imposta una nuova password" setView={setView} goBack={goBack}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <PasswordField label="Nuova password" value={password} onChange={(e) => setPassword(e.target.value)} error={errors.password} />
        <PasswordField label="Conferma nuova password" value={confirm} onChange={(e) => setConfirm(e.target.value)} error={errors.confirm} />
        {formError && <p className="text-xs" style={{ color: "#C24444" }}>{formError}</p>}
        <Button variant="primary" className="w-full" type="submit" disabled={loading}>
...
function getInitialView() {
  ...
  if (resetToken) {
    return { name: "reset-password", token: resetToken };
  }
  return { name: "home" };
}
...
export default function App() {
  ...
  // ogni navigazione "in avanti" ricorda da dove veniamo, così "Indietro"
  // funziona su qualunque pagina del sito
  ...
  return (
    <div className="min-h-screen font-sans" style={{ background: C.bg }}>
      <Header view={view} setView={setView} session={session} onLogout={() => { setSession(null); setView({ name: "home" }); }} />
      {view.name === "home" && <HomePage setView={setView} courses={courses} />}
      {view.name === "catalog" && <CatalogPage courses={courses} setView={setView} goBack={goBack} initialCategory={view.category} />}
        {view.name === "course" && (
        <CourseDetailPage
          course={activeCourse}
          setView={setView}
      ...
      {view.name === "languages" && <LanguagesPage setView={setView} goBack={goBack} />}
      {view.name === "lesson" && <LessonPage lang={activeLang} setView={setView} goBack={goBack} />}
      {view.name === "login" && <LoginPage setView={setView} goBack={goBack} onLoggedIn={setSession} />}
      {view.name === "signup" && <SignupPage setView={setView} goBack={goBack} onLoggedIn={setSession} />}
      {view.name === "forgot" && <ForgotPasswordPage setView={setView} goBack={goBack} />}
      {view.name === "reset-password" && <ResetPasswordPage token={view.token} setView={setView} goBack={goBack} />}
