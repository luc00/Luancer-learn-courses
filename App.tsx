...
import {
  ...
  Check, X, ArrowLeft, Volume2, Lock,
} from "lucide-react";
...
function Button({ children, variant = "primary", className = "", ...props }) {
  ...
  return (
    <button
      ...
      className={`px-5 py-3 rounded-md text-sm font-medium transition-opacity hover:opacity-85 ${className}`}
    >
      {children}
    </button>
  );
  ...
  CATEGORIES.forEach((cat) => {
    SUBJECT_STEMS.forEach((stem) => {
      LEVELS.forEach((lvl) => {
        ...
        list.push({
          id, title: `${stem}: ${cat}`, category: cat, level: lvl.name, premium: lvl.premium,
...
const LANGUAGES = [
  ...
  ["Irlandese", "🇮🇪"], ["Scozzese Gaelico", "🏴"], ["Basco", "🇪🇸"],
  ["Catalano", "🇪🇸"], ["Galiziano", "🇪🇸"], ["Yiddish", "🇮🇱"], ["Esperanto", "🌍"],
  ["Latino", "🏛️"], ["Antico Greco", "🏛️"], ["Haitiano Creolo", "🇭🇹"],
  ["Quechua", "🇵🇪"], ["Guarani", "🇵🇾"], ["Nahuatl", "🇲🇽"], ["Maori", "🇳🇿"],
  ["Hawaiano", "🇺🇸"], ["Samoano", "🇼🇸"], ["Fijiano", "🇫🇯"], ["Tibetano", "🏔️"],
  ["Sindhi", "🇵🇰"], ["Malayalam", "🇮🇳"], ["Kannada", "🇮🇳"], ["Oriya", "🇮🇳"],
  ["Assamese", "🇮🇳"], ["Cebuano", "🇵🇭"], ["Tagalog", "🇵🇭"], ["Xhosa", "🇿🇦"],
  ["Sesotho", "🇱🇸"], ["Wolof", "🇸🇳"], ["Lingala", "🇨🇩"], ["Tigrino", "🇪🇷"],
].map(([name, flag], i) => ({
  id: i + 1, name, flag,
...
function CourseDetailPage({ course, setView }) {
        ...
        <aside className="h-fit p-5 rounded-lg sticky top-24" style={{ background: C.card, border: `1px solid ${C.border}` }}>
          ...
          {!enrolled ? (
            <>
              ...
              <Button variant="primary" className="w-full" onClick={handleEnroll}>Iscriviti al corso</Button>
            </>
          ) : (
...
function LessonPage({ lang, setView }) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-10">
      ...
      {step === "video" && (
        <div>
          <div className="aspect-video rounded-lg flex flex-col items-center justify-center mb-4" style={{ background: C.sage900 }}>
            <button className="w-16 h-16 rounded-full flex items-center justify-center mb-3" style={{ background: C.sage500 }}>
              <Play size={26} className="ml-1" style={{ color: C.sage900 }} />
            </button>
            <p className="text-sm" style={{ color: C.sage200 }}>{lang.tutor} comunica in {lang.name}</p>
            <p className="text-xs inline-flex items-center gap-1 mt-1" style={{ color: C.sage400 }}><Volume2 size={12} /> con sottotitoli</p>
          </div>
      ...
      {step === "done" && (
        <div className="text-center py-10">
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            <Button variant="primary" onClick={() => setView({ name: "languages" })}>Prossima lingua</Button>
          </div>
        </div>
      )}
    </div>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen font-sans" style={{ background: C.bg }}>
      ...
      {view.name === "course" && <CourseDetailPage course={activeCourse} setView={setView} />}
